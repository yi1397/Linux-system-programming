...
void drawCursor(int w, int h, int x1, int y1, int x2, int y2, unsigned int *src, unsigned int *bg)
{
	int i, j;
	for(i = x1; i < x1 + 20; i++)
	{
    ...
    while(1)
    {
        ...
        if(len > 0)
        {
            ...
            if(flag&&cnt==3)
            {
				...
				x1 = curX;
				y1 = curY;
				drawCursor(1280, 720, curX, curY, fb, bg);
                //fb[curX + curY*1280] = 0xFFFFFFFF;
            }
			drawCursor(1280, 720, p_curX, p_curY, curX, curY, fb, bg);
        }
    }
