...
int fh_bmp_load(char *name,unsigned char *buffer, int x, int y, unsigned char alpha)
{
	int fd, bpp, raster, i, j, k, skip;
	unsigned char buff[4];
	unsigned char *wr_buffer = buffer + x*(y-1)*4;
	...
	switch (bpp){
		case 1: /* monochrome */
			...
			for (i=0; i<y; i++) {
				...
				wr_buffer -= x*8; /* backoff 2 lines - x*2 *3 */
			}
			break;
